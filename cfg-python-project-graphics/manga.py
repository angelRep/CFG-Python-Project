...
def text_details(manga):
    text = '        ID : {}'.format(manga['id'])
    text = text + '\n     TITLE : {}'.format(manga['title'])
    text = text + '\n      RANK : {}'.format(manga['rank'])
    text = text + '\nPOPULARITY : {}'.format(manga['popularity'])
    text = text + '\n     SCORE : {}'.format(manga['score'])
    text = text + '\n FAVORITES : {}'.format(manga['favorites'])
    text = text + '\n    AVAILABLE : {}'.format(("NO" if manga['available'] == 0 else "YES"))
    return text
