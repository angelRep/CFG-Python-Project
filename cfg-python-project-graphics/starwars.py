...
def text_details(sw_character):
    text = '         NAME : {}'.format(sw_character['name'])
    text = text + '\n       HEIGHT : {}'.format(sw_character['height'])
    text = text + '\n         MASS : {}'.format(sw_character['mass'])
    text = text + '\n No. VEHICLES : {}'.format(sw_character['vehicles'])
    text = text + '\nNo. STARSHIPS : {}'.format(sw_character['starships'])
    text = text + '\n    AVAILABLE : {}'.format(("NO" if sw_character['available'] == 0 else "YES"))
    return text
